...
void setup() {
    ...
    // --- WiFi ---
    WiFiManager wm;
  // Xóa cài đặt cũ để test (Bỏ comment dòng dưới nếu muốn reset WiFi mỗi lần khởi động)
  // wm.resetSettings(); 

  // Tạo cổng cấu hình với tên "ESP32_GATEWAY_SETUP"
  // Nếu không kết nối được WiFi cũ, ESP32 sẽ dừng ở đây và phát AP
  bool res = wm.autoConnect("ESP32_GATEWAY_SETUP", "12345678"); 

  if(!res) {
    Serial.println("Ket noi that bai, dang khoi dong lai...");
    ESP.restart();
  } else {
    Serial.println("Da ket noi WiFi thanh cong qua WiFiManager!");
  }
